/*
  ...
  - A unique constraint covering the columns `[date_creation_task]` on the table `tasks` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `date_creation_task` to the `tasks` table without a default value. This is not possible if the table is not empty.
  - Added the required column `id_status` to the `tasks` table without a default value. This is not possible if the table is not empty.
  - Added the required column `name_task` to the `tasks` table without a default value. This is not possible if the table is not empty.
...
ADD COLUMN     "date_creation_task" DATE NOT NULL,
ADD COLUMN     "description_task" CHAR(255),
ADD COLUMN     "id_status" INTEGER NOT NULL,
ADD COLUMN     "id_task" SERIAL NOT NULL,
ADD COLUMN     "name_task" CHAR(50) NOT NULL,
